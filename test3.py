...
def sentences_similarity(sentence1,sentence2):
    ...
    doc1_verbs = " ".join([token.lemma_ for token in doc1 if token.pos_ == "VERB"])
    doc1_adjs = " ".join([token.lemma_ for token in doc1 if token.pos_ == "ADJ"])
    doc1_nouns = " ".join([token.lemma_ for token in doc1 if token.pos_ == "NOUN"])

    doc2_verbs = " ".join([token.lemma_ for token in doc2 if token.pos_ == "VERB"])
    doc2_adjs = " ".join([token.lemma_ for token in doc2 if token.pos_ == "ADJ"])
